...
class DialogFlowConnector extends EventEmitter {
    constructor(audioConfig, dialogFlowConfig, id, log) {
        ...
        let { zone = "us" } = dialogFlowConfig;
        this._dialogFlowZone = zone;
        this._clientOpts = {
          credentials: dialogFlowConfig.auth,
          apiEndpoint: `${zone}-dialogflow.googleapis.com`
        }
        this._dialogFlowClient = new dialogflow.SessionsClient(this._clientOpts);
        this._dialogFlowPath = this._dialogFlowClient.projectLocationAgentSessionPath(this._dialogFlowProjectId, this._dialogFlowZone, this._dialogFlowSessionId);
        log.info('ClientOps', this._clientOpts, this._dialogFlowPath);
        ...
        */
        ...
        for (let i = 0; i < frames+1; i++) {
            timeouts.push(setTimeout(() => {
                ...
                pos = newpos
            }, i === 0 ? 0 : i * 20 - 10)); // Experiment to deal with jitter to Asterisk
        }
        timeouts.push(setTimeout(() => {
          this.log.info('Stream ended to Asterisk');
          this.emit('message', {frames, ended:true});
        }, (frames + 1) * 20));
    }
